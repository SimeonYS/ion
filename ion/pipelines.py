...
class IonPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ion.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ion`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ion WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ion (date, title, link, content)"
